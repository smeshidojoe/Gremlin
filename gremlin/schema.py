...
SECTIONS: list[Section] = [
    ...
    Section(
        ...
        "чем-то насторожил.\n"
        "«Смотреть на участников чата» — выключите, и профили тех, кто в чате "
        "состоит, бот трогать перестанет: останутся комментаторы со стороны и "
        "новички на входе. Своих это бережёт — у старожила «18+» в описании "
        "его же канала это просто его канал, а не реклама.\n"
        ...
        "стоп-слов чата. Он и должен быть другим: в сообщениях запрещают темы, "
        "а в описании то же слово ловит и того, кто тему осуждает. Сюда идут "
        "рекламные метки — «в лс», «онлифанс», «18+». Когда наберётся достаточно "
        "спам-профилей, проверку словами можно выключить и оставить сравнение.\n"
        "«Смотреть на аватарку» — половина таких аккаунтов вообще ничего "
        "не пишет в описании, вся суть у них на фото. Но сама по себе "
        "аватарка никого не наказывает: распознавание путает «эффектно» с "
        "«откровенно», и на обычном портрете оно уже ошибалось. Поэтому "
        "фото только добавляет очки наблюдению — вместе с остальным о "
        "человеке. Работает, только если владелец бота поставил "
    ...
    Section(
        ...
        "Пускать в чат только тех, кто подписан на канал.\n"
        "Работает через заявки: человек нажимает «Вступить», бот смотрит, "
        "подписан ли он, и решает. Чтобы заявки вообще приходили, в самой "
        "группе должна быть включена проверка новых участников — это "
        "настройка Telegram, не наша.\n"
        "Ещё бот должен быть админом в канале: иначе спросить, подписан ли "
        "человек, не у кого.\n"
        "«Подписан» — что делать с тем, у кого подписка есть:\n"
        "• <b>впустить</b> — бот одобряет заявку сам;\n"
        "• <b>не трогать</b> — заявка остаётся вам: бот только отсекает "
        "неподписанных, а кого пускать из остальных, решаете вы. Кнопка "
        "«Я подписался» в этом режиме тоже не впускает;\n"
        "• <b>только по кнопке</b> — сам бот не одобряет, но того, кто "
        "подписался по его просьбе и нажал «Я подписался», впустит. Смысл "
        "есть только вместе с «держать и ждать» и сообщением в личку — "
        "кнопка живёт там;\n"
        "• <b>отклонить</b> — вход закрыт вообще всем, подписка не поможет. "
        "Это на время: набежала волна, чат закрыли, потом вернули «впустить». "
        "Проверку при этом выключать не надо.\n"
        "«Не подписан» — отклонить заявку сразу или придержать её.\n"
        "Отказ работает молча: заявка отклоняется, а вам приходит карточка "
        "в лог-чат. Это самый простой способ против ботов — им объяснять "
        "нечего.\n"
        "Ожидание — для живых людей: бот пишет человеку в личку со ссылкой "
        "на канал и кнопкой «Я подписался», тот подписывается, жмёт кнопку, "
        "и бот впускает его сам. Поэтому сообщение настраивается только "
        "здесь: в режиме отказа писать некому и незачем.\n"
        "Отклонённую заявку придётся подавать заново, придержанная ждёт "
        "три часа.\n"
        "Сообщение в личку — обычная заготовка: текст, эмодзи, картинка. "
        "Telegram разрешает боту написать подавшему заявку, даже если тот "
        "боту никогда не писал.",
        fields=[
    ...
    Section(
        ...
        "Набег — это когда в чат за минуту вливается толпа: разбирать каждого "
        "по отдельности поздно, надо на время закрыть дверь.\n"
        "Бот считает входы и, когда их становится больше порога, включает "
        "режим: каждый, кто входит дальше, получает выбранное наказание, а в "
        "лог-чат уходит карточка со списком всех вошедших — по нему видно, кто "
        "это был.\n"
        "В карточке две кнопки: снять режим досрочно и забанить всех, кто "
        "вошёл за набег.\n"
        "Режим снимается сам по истечении срока, а также при перезапуске бота.\n"
        "Штатный «вход по заявке» Telegram боту включать не даёт, поэтому "
        "защита работает мутом, киком или капчей.",
        fields=[
    ...
    Section(
        "report", "🚨 Жалобы",
        "Участник отвечает на чужое сообщение командой <code>!report</code> "
        "(или <code>!жалоба</code>, <code>@admin</code>) — и оно уходит админам "
        "карточкой в лог-чат.\n"
        "Причину писать необязательно: в карточке и так видно само сообщение, "
        "на кого жалуются и кто пожаловался.\n"
        "Кнопки в карточке: удалить сообщение, мут, бан, записать профиль в "
        "базу спама или отклонить жалобу.\n"
        "Саму команду бот убирает из чата — чтобы не было видно, кто "
        "пожаловался.\n"
        "«Жалобы на админов» — слать ли карточку, когда жалуются на админа "
        "чата. По умолчанию такие бот молча пропускает.\n"
        "Повторные жалобы на одно сообщение новых карточек не плодят: в первой "
        "растёт счётчик.",
        fields=[
...
GROUP_OF = {
    ...
    "sub": "people", "report": "people", "raid": "people",
