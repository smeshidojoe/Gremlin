...
SECTIONS: list[Section] = [
    ...
    Section(
        ...
        "Наказание задаётся отдельно для каждого типа и отдельно для тех, кто "
        "в чате не состоит — кнопки ниже.",
        fields=[
    ...
    Section(
        ...
        "Что делать с теми, кто в чате состоит. У каждого типа своё наказание — "
        "например, ссылку на чужой канал удалять, а за пересылку мутить.",
        fields=[
    ...
    Section(
        ...
        "Для тех, кто в чате не состоит — обычно это комментаторы под постами "
        "канала. Именно отсюда идёт почти вся реклама, поэтому наказания тут "
        "обычно строже: например, бан за телеграм-ссылку.",
        fields=[
    ...
    Section(
        ...
        "Звёздочка на конце ловит любые окончания: <code>пив*</code> сработает на "
        "«пиво», «пива», «пивом». "
    ...
    Section(
        ...
        "<i>Ловить в любом месте</i> — по умолчанию счётчик срабатывает, только если "
        "сообщение с него начинается. Включите, чтобы команда ловилась и в середине "
        "текста; тогда она сработает и при обсуждении самой команды.\n"
    ...
    Section(
        "warns", "⚠️ Варны",
        "<i>Зачем:</i> предупреждение вместо сразу бана — для своих, кто "
        "перегнул палку.\n"
        "<i>Как:</i> админ отвечает <code>!warn</code> (или <code>!варн</code>) на сообщение. "
        "Сообщение нарушителя и сама команда удаляются, в лог уходит карточка "
        "со счётом. Набрал лимит — прилетает наказание, счёт обнуляется.\n"
        "Сами варны не сгорают — снять можно вручную в списке ниже.",
        fields=[
    ...
    Section(
        ...
        "<i>Зачем:</i> первым комментарием под каждым постом канала "
        "висят правила — новички видят их сразу.\n"
        "<i>Как:</i> бот ждёт автопересылку поста из привязанного канала и через "
        "пару секунд отвечает на неё заготовкой. Ничего не закрепляется "
        "и не удаляется: под каждым постом свой комментарий.\n"
        "Заготовок может быть несколько — тогда бот берёт случайную. "
        "Работает только в чате, привязанном к каналу.",
        fields=[Field("rules_on", "toggle", "Статус")],
        widgets=["rules_text"],
    ),
    Section(
        "punish_cfg", "⚙️ Настройки наказаний",
        "<i>Команды в чате:</i> <code>!ban</code>, <code>!mute</code>, <code>!dm</code>, "
        "<code>!unban</code>, <code>!unmute</code> — работают у админов чата, ответом "
        "на сообщение или с указанием @ника либо id.\n"
        "<i>Мут за чужие команды</i> — что сделать, если команду пишет обычный "
        "участник: сообщение удаляется, а сам он получает мут на это время. "
        "Люди из вайтлиста и админы не трогаются.",
        fields=[
...
OVERVIEW = [
    ...
    ("rules_on", "Правила"), ("cards_on", "Карточки"),
]
